...
- The mgr13 overlay stays inside the current screen bounds at the selected scale; style labels have reserved space and all active advanced passes default open.

To apply mgr13 to an existing managed game, close the game, open it in the new manager build and choose **Update in-game backend**, then restart the game. Installing a newer manager alone does not replace a backend already copied into a game folder.
